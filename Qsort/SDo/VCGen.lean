...
namespace Lean.Elab.Tactic.SDo
...
private def _root_.Lean.Elab.Tactic.Do.ProofMode.MGoal.withNewProg (goal : MGoal) (e : Expr) : MGoal :=
...
open Lean.Elab
open Lean.Elab.Tactic.Do.VCGen
open Lean.Elab.Tactic.Do.ProofMode
open Lean.Elab.Tactic.Do.ProofMode.MGoal
namespace VCGen
...
partial def genVCs (goal : MVarId) (ctx : Do.Context) (fuel : Fuel) : MetaM Result := do
...
where
  ...
  onGoal goal name : VCGenM Expr := do
    ...
    let f := T.getAppFn
    trace[Elab.Tactic.Do.vcgen] "f: {f}"
  ...
  onWPApp goal name : VCGenM Expr := ifOutOfFuel (onFail 2 goal name) do
    ...
    if let .letE x ty val body _nonDep := e.getAppFn' then
      burnOne
      trace[Elab.Tactic.Do.vcgen] "e: {e}, {e.getAppNumArgs'}"
...
syntax (name := smvcgen) "smvcgen" optConfig
