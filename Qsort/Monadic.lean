...
@[inline] def qpartition' (as : {as : Array α // as.size = n})
    ...
    Id $ {as : Array α // as.size = n} × {pivot : Fin n // lo ≤ pivot ∧ pivot ≤ hi} := do
...
-- def Vector.get (as : Vector α n) (i : Fin n) := as.1[(i.cast as.2.symm)]
--
...
--   as.1 ~ as'.1 ∧ ∀ i : Fin n, ¬(lo ≤ i ∧ i < hi) → as.get i = as'.get i
--
...
--   --     some (as.1.toList.get (Fin.mk k _)) = some (as'.1.toList.get (Fin.mk k _)) := congrArg _ (H.2 ⟨k, h⟩ hk)
--   -- simp only [← List.get?_eq_get, eq, eq', List.get?_eq_getElem?] at H2
...
--   --   rw [← get?_eq_get, ← get?_eq_get, List.get?_eq_getElem?, List.get?_eq_getElem?]
--   --
--   --   have i_lo := h1 ▸ hi1
--   --   have := H2 (Nat.lt_of_lt_of_le i_lo (Nat.le_trans lo_hi hi_n)) fun h => Nat.not_le.2 i_lo h.1
--   --   rwa [List.getElem?_append_left, List.getElem?_append_left hi1, List.getElem?_append_left, List.getElem?_append_left hi2] at this
--   --     <;> simp; omega
--   -- have eq₃ : L₃ = L₃' := by
...
--   let x := @getElem_swap' _ as.val i j sorry sorry k sorry
--   simp [Vector.get, Vector.swap, x]
--   -- split <;> [skip; split] -- FIXME why doesn't this work anymore?
--   split
...
--   · rfl
--
-- def df (n : Nat) : Nat := 5 + n
-- axiom ax (n : Nat) : 5 + n = n + 5
-- theorem thm (n : Nat) : df n = n + 5 := by
--   apply ax
--
--
