...
attribute [sort] Fin.getElem_fin Array.length_toList Array.size_swap List.append_left_inj List.append_right_inj List.length_cons ArrayToList.length_swap
...
theorem qpartition_sorted (le_asymm : ∀ {{a b}}, lt a b → ¬lt b a) (le_trans : ∀ {{a b c}}, ¬lt a b → ¬lt b c → ¬lt a c)
  ...
  . next i j _ _ _ _ s hhihj =>
    ...
    -- let x? : Nat := ?_
    if h' : x = i then -- FIXME want to figure out `i` through unification
      subst h'
      apply le_asymm
